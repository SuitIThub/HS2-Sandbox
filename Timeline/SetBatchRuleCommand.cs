...
{
    /// <summary>
    /// Sets a CopyScript batch rule by tag name via API. UI matches CopyScript rule row: tag + start, increment, step, max. On success refreshes CopyScript window; on failure shows Resolve.
    /// </summary>
    public class SetBatchRuleCommand : TimelineCommand
    {
        private const char PayloadSeparator = '\u0001';

        public override string TypeId => "set_rule_batch";
        ...
        public override string GetDisplayLabel() => "Rule (batch)";
